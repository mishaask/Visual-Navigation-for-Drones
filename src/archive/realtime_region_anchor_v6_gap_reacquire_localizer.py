...
def main() -> None:
    ...
    print(f'region-anchor v6-gap-reacquire: acquire {args.acquire_min_region_votes}/{args.acquire_window}, fail_limit={args.fail_limit}')
    ...
    pbar=tqdm(desc='region-anchor-v6-gap-reacquire realtime frames')
    while True:
        ok, frame=cap.read()
        if not ok: break
        frame_idx += 1
