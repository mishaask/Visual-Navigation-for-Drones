"""Region-anchor realtime localizer v5: safe flow + landmark/pose diagnostics.
...
def main() -> None:
    ...
    ap.add_argument('--max-flow-hold-speed-mps', type=float, default=18.0)
    ...
    print(f'region-anchor v5-safe-landmark: acquire {args.acquire_min_region_votes}/{args.acquire_window}, fail_limit={args.fail_limit}')
    ...
    pbar=tqdm(desc='region-anchor-v5-safe-landmark realtime frames')
    while True:
        ok, frame=cap.read()
        if not ok: break
        frame_idx += 1
        ...
        flow_speed_m_s=float(flow['mag_px'])*active_mpp/max(dt_sample,1e-6)
        ...
        extra={
            ...
            'projected_center_offset_frac': getattr(best, 'projected_center_offset_frac', math.nan),
            'landmark_pose_mismatch_warning': int(math.isfinite(getattr(best, 'projected_center_offset_frac', math.nan)) and getattr(best, 'projected_center_offset_frac', math.nan) > args.projected_center_offset_warning_frac),
            'flow_raw_speed_m_s': flow_speed_m_s,
            'flow_speed_capped_for_hold_m_s': min(args.max_flow_hold_speed_mps, flow_speed_m_s) if math.isfinite(flow_speed_m_s) else '',
